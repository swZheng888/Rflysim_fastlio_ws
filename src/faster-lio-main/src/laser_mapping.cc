...
bool LaserMapping::InitWithoutROS(const std::string &config_yaml)
{
  ...
  if(std::is_same<IVoxType, IVox<3, IVoxNodeType::PHC, pcl::PointXYZI>>::value
     == true)
  {
    LOG(INFO) << "using phc ivox";
  }
  else if(std::is_same<IVoxType,
                       IVox<3, IVoxNodeType::DEFAULT, pcl::PointXYZI>>::value
          == true)
  {
...
void LaserMapping::Run()
{
  ...
  Timer::Evaluate(
    [&, this]() {
      ...
      // update the observation model, will call nn and point-to-plane residual
      // computation
    ...
    std::execution::unseq, index.begin(), index.end(), [&](const size_t &i) {
      ...
      if(!nearest_points_[i].empty() && flg_EKF_inited_)
      {
        ...
        if(points_near.size() >= options::NUM_MATCH_POINTS)
        {
          ...
          {
            if(common::calc_dist(points_near[readd_i].getVector3fMap(), center)
               < dist + 1e-6)
            {
              need_add = false;
              break;
        ...
        [&](const size_t &i) {
          ...
          if(extrinsic_est_en_)
          {
            ...
            ekfom_data.h_x.block<1, 12>(i, 0) << norm_vec[0], norm_vec[1],
              norm_vec[2], A[0], A[1], A[2], B[0], B[1], B[2], C[0], C[1], C[2];
          }
          else
          {
            ekfom_data.h_x.block<1, 12>(i, 0) << norm_vec[0], norm_vec[1],
...
void LaserMapping::PublishOdometry(const ros::Publisher &pub_odom_aft_mapped)
{
  ...
  br.sendTransform(tf::StampedTransform(
    transform, odom_aft_mapped_.header.stamp, tf_world_frame_, tf_imu_frame_));
}
...
///////////////////////////  private method
////////////////////////////////////////////////////////////////////////
template <typename T> void LaserMapping::SetPosestamp(T &out)
{
